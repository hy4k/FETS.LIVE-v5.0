...
export function SevenDayExamOutlook({
  ...
  const rosterHint =
    activeBranch === 'global'
      ? 'FETS roster · all centres'
      : `FETS roster · ${formatBranchName(activeBranch)}`
    ...
    >
      <div className="px-5 md:px-8 pt-6 md:pt-8 pb-4 border-b border-white/[0.06]">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-sm bg-[#FACC15]/10 border border-[#FACC15]/25 flex items-center justify-center">
            <Calendar size={18} className="text-[#FACC15]" aria-hidden />
          </div>
          <div>
            <h2 className="text-lg md:text-xl font-black text-white uppercase tracking-[0.15em] leading-tight">
              7-Day Exam Outlook
            </h2>
            <p className="text-xs md:text-sm text-[#FACC15]/60 uppercase tracking-widest font-bold mt-1.5">{subtitle}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex overflow-x-auto snap-x snap-mandatory md:snap-none scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent bg-black/[0.08]">
        {dayKeys.map((ymd) => {
          ...
          const rosterFirstNames = staffByDate[ymd] || []
          ...
          return (
            <div
              key={ymd}
              className="snap-start shrink-0 w-[min(100%,260px)] md:flex-1 md:min-w-0 border-r border-white/[0.06] last:border-r-0 flex flex-col min-h-[420px] md:min-h-[460px]"
            >
              <div
                className={`shrink-0 px-4 py-3 border-b border-white/[0.06] ${
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  {isToday && (
                    <span className="text-[10px] font-black uppercase tracking-[0.12em] px-2 py-0.5 rounded-md bg-[#FACC15]/15 text-[#FACC15] border border-[#FACC15]/30">
                      Today
                    </span>
                  )}
                </div>
              ...
              <div className="shrink-0 border-t border-white/[0.08] bg-black/25 p-4 mt-auto">
                ...
                <p className="text-[9px] md:text-[10px] text-white/35 font-bold uppercase tracking-widest mb-2">{rosterHint}</p>
                {staffLoading ? (
                  <div className="text-xs text-white/40 font-medium">Loading roster…</div>
                ) : rosterFirstNames.length === 0 ? (
                  <div className="text-xs md:text-sm text-white/40 font-medium">None scheduled</div>
                ) : (
                  <div className="flex flex-wrap gap-1.5">
                    {rosterFirstNames.map((name) => (
                      <span
                        key={`${ymd}-${name}`}
                        className="inline-flex items-center px-2.5 py-1 rounded-md text-xs md:text-sm font-semibold text-white/90 bg-white/[0.04] border border-white/[0.08]"
                      >
                        {name}
                      </span>
                    ))}
